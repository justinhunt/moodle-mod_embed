...
class attempts extends basereport
{
    ...
    public function fetch_formatted_field($field,$record,$withlinks)
    {
        ...
        switch ($field) {
            ...
            case 'view':
                if ($withlinks && has_capability('mod/embed:manageattempts', $this->context)) {
    ...
    public function process_raw_data($formdata){
        ...
        if($formdata->groupid>0){
            ...
            $sql .= ' WHERE gm.groupid ' . $groupswhere . ' AND at.embed = ?';
            $sql .= ' ORDER BY at.timemodified DESC';
            $allparams[]=$formdata->embedid;
            ...
            $sql .= ' WHERE at.embed = :embedid';
            $sql .= ' ORDER BY at.timemodified DESC';
            $alldata = $DB->get_records_sql($sql,array('embedid'=>$formdata->embedid));
        }
